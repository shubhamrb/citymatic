...
public class ScheduleFragmentViewModel extends BaseViewModel<ScheduleFragmentNavigator> {

    public ScheduleFragmentViewModel(IDataManager dataManager, ISchedulerProvider schedulerProvider) {
        super(dataManager, schedulerProvider);
    }

    public void fetchBookingDates(Activity mActivity) {
        if (NetworkUtils.isNetworkConnected(mActivity)) {
//            getmNavigator().get().showProgressBars();
            getmDataManger().fetchBookingDates(mActivity, getmDataManger().getAccessToken(), new ResponseListener() {
                @Override
                public void onSuccess(JsonObject jsonObject) {
                    try {
//                        getmNavigator().get().hideProgressBars();
                        getmNavigator().get().onSuccessBookingDates(jsonObject);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
